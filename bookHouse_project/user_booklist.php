...
<?php
  ...
  $sql = "SELECT * FROM user_booklist INNER JOIN booklist ON user_booklist.book_name = booklist.book_name where user_id='$user_id' AND user_booklist.choice_list!='None'";
...
<div class="read">
 ...
 <?php  
  ...
  if ($_SERVER["REQUEST_METHOD"]=="POST") {
    ...
    $share_file = $_POST["file"];

  $sql = "INSERT INTO user_share_book (`book_share_id`, `user_id`, `share_book_name`, `book_author`, `share_file`, `size`)  VALUES ('','$user_id','$sbook_name','$author_name','$share_file','')";
...
<div class="booklist">
<span class="error">* <?php echo $fileError;?></span></tr>
        ...
        if (mysqli_num_rows($result) > 0 ) {
         while($row = mysqli_fetch_array($result)){
            ...
            <div class="book_name"><?php echo $row['book_name'];?></div>
             ...
             <td>
             <div class="rating"><?php echo number_format($row['book_rating']);?></div>
             </td>
             <td>
...
function selectStatus(e) {
       ...
       var book_id = $(e).attr('book_id');
        $.ajax({
                ...
                alert("Refresh page for updated list");
                 }
      })
}
</script>
